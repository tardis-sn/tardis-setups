...
def run_tardis_model(params, pickled=False):
    ...
    sim.run()
    fname = 'Output/Toy_01/toy01_t{}_v{}.hdf'.format(
    ...
    if pickled:
        ...
        if "PICKLE_DIR" not in os.environ:
            dump = 'Output/Toy_01/toy01_t{}_v{}.pickle'.format(
                params[0].value, params[2].value)
        else:
            dump = os.path.join(os.environ['PICKLE_DIR'], 'toy01_t{}_v{}.pickle'.format(
                params[0].value, params[2].value))
...
if 'ATOM_DATA' in os.environ:
    parse_config('blondin_model_compare_01.yml')
else:
    print('\n*** error! *** ATOM_DATA environment variable is not set.')
    sys.exit(1)

final_params = [(5*u.d, lbols[0],  19000.*u.km/u.s),
                (10*u.d, lbols[1], 14000.*u.km/u.s),
                (15*u.d, lbols[2], 11000.*u.km/u.s),
                (20*u.d, lbols[3], 9000.*u.km/u.s)]
